//
//  SJClipsVideoRecordsControlLayer.swift
//  SJVideoPlayer
//
...
@objc(SJClipsVideoRecordsControlLayer)
@MainActor
public class SJClipsVideoRecordsControlLayer: SJEdgeControlLayerAdapters, SJControlLayer {
    ...
    @objc public var statusDidChangeExeBlock: ((SJClipsVideoRecordsControlLayer) -> Void)?
    ...
    private var countDownView: SJClipsVideoCountDownView!
    private var countDownTimer: Timer?
    private var countDownNum: Int = 60 * 2 {
        didSet {
            if countDownNum == oldValue { return }
            _updateBottomItemSettings()
        }
    }
    private let maxCountDownNum: Int = 60 * 2
    ...
    #if DEBUG
    deinit {
        print("\(#line) - -[SJClipsVideoRecordsControlLayer dealloc]")
    }
    #endif
    ...
    private func _addItemToBottomAdapter() {
        ...
        let view = SJClipsVideoCountDownView(frame: .zero)
        countDownView = view
    ...
    private func _updateBottomItemSettings() {
        let strings = SJVideoPlayerConfigurations.shared.localizedStrings
        let current = player?.stringForSeconds(maxCountDownNum - countDownNum) ?? "00:00"
        let max = player?.stringForSeconds(maxCountDownNum) ?? "00:00"
        countDownView.timeLabel.text = "\(current)/\(max)"
        ...
        countDownView.progressSlider.maxValue = CGFloat(maxCountDownNum)
        countDownView.progressSlider.value = CGFloat(maxCountDownNum - countDownNum)
